...
public:
  ...
  struct CRINFO// GC to LC
  {
    Ipv4Address ID,sourceAddress,destAddress,lastAddress;
    uint32_t m_lc_info_p;
    uint32_t m_lc_info_n;
  ...
  struct LCINFO
  {
    ...
    uint32_t p, n;//道路两个方向是否有链路
    uint32_t x, y; //LC location
    Ipv4Address m_lc_info_p_begin;
    Ipv4Address m_lc_info_p_end;
    Ipv4Address m_lc_info_n_begin;
    Ipv4Address m_lc_info_n_end;
    std::map<Ipv4Address, CarInfo> m_gc_info;//在该lc区域的所有车辆
...
public:
  ...
  LCINFO& GetLCINFO()
  {
    if (m_messageType == 0)
      {
        m_messageType = LC_INFO_MESSAGE;
      }
    else
      {
        NS_ASSERT (m_messageType ==LC_INFO_MESSAGE);
      }
    return (m_message.lcinfo);
  }
  ...
  CRINFO& GetCRINFO()
  {
    if (m_messageType == 0)
      {
        m_messageType = CRINFO_MESSAGE;
      }
    else
      {
        NS_ASSERT (m_messageType ==CRINFO_MESSAGE);
      }
    return (m_message.crinfo);
  }
  ...
  const LCINFO& GetLCINFO() const
  {
    NS_ASSERT (m_messageType == LC_INFO_MESSAGE);
    return (m_message.lcinfo);
  }

  const CRINFO& GetCRINFO() const
  {
    NS_ASSERT (m_messageType == CRINFO_MESSAGE);
    return (m_message.crinfo);
  }
